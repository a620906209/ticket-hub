...
- **WHEN** 訂單確認的付款、座位確售、票券產出皆成功，但通知流程因這次 `ConfirmOrderAsync` 呼叫本身的 `CancellationToken` 被觸發而中止
...
- **WHEN** 要寫入 log 的收件信箱欄位為 `null`、空字串、純 whitespace，或不含 `@` 的不合法格式
- **THEN** 遮蔽動作 MUST NOT 拋出例外，log 內容 SHALL 使用固定的替代字串，MUST NOT 原樣輸出該不合法內容
