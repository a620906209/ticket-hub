...
- **[Risk][高，外部審查在實作完成後抓到] Migration 的 Down() 一旦已有純計數訂單資料，rollback 必定因 FK violation 失敗** → `OrderItems.EventSeatId` 改回 NOT NULL 時，既有的 `EventSeatId IS NULL` 資料列會被批次填成 `Guid.Empty`，但這個值不對應任何真實 `EventSeat`、違反 FK 約束；已用 `dotnet ef database update` 對已有純計數訂單的 dev DB 實際測試過會失敗。Mitigation：Down() 開頭加上明確的前置檢查（偵測到 `EventSeatId IS NULL` 就 `RAISE EXCEPTION` 並說明原因），避免部署人員誤以為這是一支「純新增欄位、可安全 down」的 migration；真的要回滾必須先做受控資料遷移。**先前 Migration Plan 第 3 點「理論上可安全 down-migration」的判斷已修正為不成立**
- **[Risk][中，外部審查在實作完成後抓到，共兩輪] 跨活動驗證原本排在資料庫鎖之後，違反 ticket-ordering spec「驗證後才鎖」的規定** → 第一輪修正：`OrderService.PlaceOrderAsync` 原本只用「第一個票種的活動」查限購張數，真正的跨活動檢查延後到 `CreateOrderHandler.Handle` 才做，改成在交易開始前對 `ticketTypesById.Values` 的 `EventId` 做 `Distinct()` 檢查。**第二輪修正（外部審查再抓到一次）**：只比對票種的活動還不夠——只有「一個票種」時（例如一個綁座位票種配另一場活動的座位），這個比對會通過，直到座位被 `GetForUpdateAsync` 鎖定後才被後面的 `ticketType.EventId != eventSeat.EventId` 擋下。修正為新增 `IEventSeatRepository.GetByIdsAsync`（no-tracking 批次查詢，跟 `TicketTypeRepository.GetByIdAsync` 同一個道理，避免後續 `GetForUpdateAsync` 撞上 identity resolution 舊快照），把座位所屬活動也併入鎖定前的比對集合——「票種活動∪座位活動」聯集大小為 1，數學上就保證每組座位與其配對票種的活動必然相同，不需要再額外逐一比對
- **[Risk][中，外部審查在實作完成後抓到] `Quantity` 加總用 `int` 累加會被外部輸入觸發 `OverflowException`（500 而非驗證錯誤）** → `PlaceOrderRequestValidator` 只驗證 `Quantity >= 1`，沒有上限；`Enumerable.Sum(int)` 是 checked 累加，兩個接近 `int.MaxValue` 的 `Quantity` 會讓限購張數的加總直接拋例外。Mitigation：改用 `Sum(s => (long)s.Quantity)` 累加
...
3. **⚠️（修正，外部審查抓到）「理論上可安全 down-migration」的判斷不成立**：先前的說法是「本次都是新增欄位／改寬鬆，理論上可安全 down」，但這只考慮了「schema 層面新增／放寬」，沒考慮到 `EventSeatId` 這一欄從 nullable 改回 NOT NULL 時，一旦已經有純計數訂單（`EventSeatId IS NULL`）產生，Down migration 會需要把這些 NULL 值批次填成某個值——而 `EventSeatId` 有 FK 約束指向 `EventSeats`，沒有一個「安全的」值可以填（`Guid.Empty` 不對應任何真實座位，會直接違反 FK）。這代表**這支 migration 一旦有純計數訂單資料產生，就不可安全 down-migrate**，MUST 視為單向遷移：Down() 已加上明確的前置檢查（偵測到 `EventSeatId IS NULL` 的資料就直接 `RAISE EXCEPTION` 並說明原因），避免部署人員誤以為 rollback 安全、卻在 FK violation 這種難懂的錯誤訊息前卡住。若真的需要回滾，必須先做受控的資料遷移（例如先處理/退款這些純計數訂單），不能直接跑 `dotnet ef database update` 的 down 路徑
