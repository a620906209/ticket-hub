...
- [x] 4.9 `ProjectC.WebApi.Tests`：`AdminOrdersControllerTests` 涵蓋 spec「查看訂單需要 Admin 角色」（Admin 成功、非 Admin 403、未登入 401，**列表與明細兩個端點都要各測一次，不能只測列表**——`[Authorize]` 雖然標註在 Controller 類別上會同時保護兩個端點，但測試本身仍要對明細端點單獨驗證，不能只靠列表端點的測試間接帶過）、「查詢所有訂單列表」、「查詢單筆訂單明細」（含查詢不存在的訂單 404）全部 Scenario
...
- [x] 5.11 確認 `CleanupOnceAsync` 的 `CreateScope()`/`GetRequiredService<OrderService>()`/`CancelExpiredOrderAsync(...)` 三步驟都在單筆 `try` 區塊內（不是宣告在 `try` 外面）——開 Scope 或解析服務本身失敗，也 MUST 被當成單筆訂單失敗處理、不中斷 `foreach` 剩餘訂單
