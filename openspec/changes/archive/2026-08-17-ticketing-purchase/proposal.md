...
- 新增買家端瀏覽 API（不需登入）：`GET /api/events`（活動列表）、`GET /api/events/{id}/seats`（座位可售狀態，含分區代碼供比對票種）、`GET /api/events/{id}/ticket-types`（票種與價格）；活動不存在時三個端點皆回 404。
- 新增訂單 API（需登入）：`POST /api/orders`（建立訂單，選定一組座位+票種配對）、`POST /api/orders/{id}/confirm`（確認，模擬付款成功）、`POST /api/orders/{id}/cancel`（取消）。建立訂單時新增「座位所屬分區須與所選票種的分區一致」驗證，避免用低價分區的票種配高價分區的座位。
...
- 新增 `OrderService` 協調服務，把 `ticketing-infrastructure` 已就緒的 Repository/`IUnitOfWork`/`EventSeat` 悲觀鎖跟既有三個 pure Handler（`CreateOrderHandler`/`ConfirmOrderHandler`/`CancelOrderHandler`，來自 `ticketing-core-domain`）串起來。**確認/取消訂單在取得座位鎖之後，會重新讀取訂單最新狀態才繼續**，避免兩筆並發操作（尤其是兩個並發取消同一筆訂單——這是唯一真正依賴這個重讀機制的情境，見 design.md 決策 3）因為等鎖期間訂單資料已經過期而互相覆蓋。
...
### New Capabilities
- `ticket-purchase`：買家端 API，任何人（不需登入）皆可瀏覽活動與座位可售狀態、票種價格；已登入會員可建立訂單、確認訂單、取消訂單。
...
- 新增 `ProjectC.Application.Orders.OrderService`（協調服務，依賴既有的 `ISeatMapRepository`/`IEventRepository` 做分區比對）、`PlaceOrderRequestValidator`，與對應的瀏覽用 Query Handler。
