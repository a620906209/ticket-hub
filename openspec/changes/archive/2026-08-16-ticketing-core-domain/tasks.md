...
- [x] 1.4 在 `ProjectC.Domain` 定義領域例外：`SeatAlreadyHeldException`、`SeatNotHeldException`、`SeatAlreadySoldException`、`OrderNotPendingException`（供 Entity 守衛方法拋出）。**[2026-08-15 更新]** 原本的 `OrderAlreadyConfirmedException` 在驗收時被 `OrderNotPendingException` 取代並移除，見 design.md Decision 19
...
- [x] 3.7 確認 `EventSeat` 內部欄位不對外暴露可直接讀取判斷可售性的成員，僅公開 `GetStatus` / `IsAvailableForHold` / `IsHeldBy` / `IsSoldBy`（**[2026-08-15 驗收時新增]**，見 design.md Decision 20）與上述操作方法
...
- [x] 4.5 實作 `CancelOrderHandler`：接受任何內部 `Status == Pending` 的訂單（不論 `GetStatus(now)` 是 Pending 或 Expired），`Status != Pending`（含 Confirmed、Cancelled）一律回傳失敗 `Result`。**[2026-08-15 驗收修正兩輪，見 design.md Decision 20]** 對訂單內每筆 `OrderItem` 對應的 `EventSeat`：若已標記 Sold 且售出訂單編號正是本訂單（`IsSoldBy(order.Id)`），視為不一致狀態、整單拒絕；若已標記 Sold 但售出訂單編號是別人，略過該座位不呼叫 `ReleaseHold`；其餘情況呼叫 `ReleaseHold(order.Id)`（已被其他訂單重新鎖定的座位維持 no-op 不釋放）。全部處理完後將 `Order.Status` 設為 Cancelled。此 Handler 同時涵蓋「使用者主動取消」與「查詢後發現已逾時、需要清理」兩種呼叫情境，不另外開 `ExpireOrderHandler`
...
- [x] 5.3 執行全部測試並確認通過。**[2026-08-15 更新]** 合併 `feature/membership-system` 後，`master` 上 `docker-compose.yml` 已存在（`api`/`db` 服務齊全），但本次驗收修復仍延續先前的例外，直接用本機 `dotnet test` 執行（容器未啟動，尚未實際切換）；下次有機會時應改回 `docker compose exec api dotnet test` 驗證，確認容器環境下行為一致。目前全 repo 測試：Domain.Tests 51、Application.Tests 43、WebApi.Tests 23（WebApi.Tests 為會員系統既有測試，非本 change 產出），全數通過。**[2026-08-16 補做]** 改用 `docker compose exec api dotnet test` 在容器內驗證，過程中發現並修掉四層環境問題（詳見 `docker-compose.yml` 與 `CustomWebApplicationFactory.cs` 的對應註解，非本 change 範圍，屬會員系統既有整合測試的容器化缺口）：① `api` 容器原本連不到 host 的 Docker（Testcontainers 需要），補掛 `/var/run/docker.sock`（Docker-out-of-Docker，僅限本機開發）；② Docker Desktop for Windows 下，`api` 所在的 compose 網路跟 Testcontainers 建立的暫存容器所在網路互相隔離，連 host-published port 都連不到，改為把兩者固定掛在同一個網路（`projectc_default`）並用網路別名直連；③ Testcontainers 的 Ryuk（自動回收孤兒容器）在此情境下同樣連不到，官方對此已知限制的建議是停用，已停用（`DisposeAsync()` 本身已正確清理，僅測試行程被強制中斷才會留孤兒容器）；④ 修正過程中一度把網路別名寫死成固定字串，導致 xUnit 平行跑測試類別時多個容器搶同一別名、連線被導到別的測試類別資料庫（間歇性 500），已改為每個 `CustomWebApplicationFactory` 實例用 GUID 產生唯一別名。容器內驗證結果：Domain.Tests 53、Application.Tests 45、WebApi.Tests 23，全數通過，跑兩輪確認穩定非偶然
- [x] 5.4 比對 tasks 完成狀況與三份 spec 的 Scenario，皆有對應測試。**[2026-08-15 驗收修正，第一輪]** 驗收時發現並補上的缺口：`CancelOrderHandler` 未攔截 `ReleaseHold` 拋出的 `SeatAlreadySoldException`（可能造成部分釋放與例外穿透）、`Cancel()` 允許對已取消訂單重複取消、`Confirm()`/`Cancel()` 未使用具名領域例外、`CreateOrderHandler` 未驗證座位/票種是否同屬一場活動、未擋重複選取同一座位、`TicketType` 建立時未核對座位圖確實屬於該活動——以上皆已修正並補上對應測試（見 design.md Decision 17-20）。**[2026-08-15 驗收修正，第二輪]** 第一輪修的 `CancelOrderHandler` 預檢邏輯本身跟 Decision 10 的設計意圖矛盾（把「座位已被別人合法賣掉」也當成整單失敗，導致逾時清理永遠卡住）；改為只在「座位已由本訂單自己售出」時才拒絕，「被別人合法售出」則略過該座位、繼續完成取消（新增 `EventSeat.IsSoldBy`，見 design.md Decision 20 第二版）。另外確認 `ConfirmOrderHandler` 的 `ConfirmSold` 迴圈刻意不包 try/catch 是有意的設計決定，非遺漏（見 design.md Decision 21：Sold 不可逆、沒有補償操作，包 try/catch 反而會製造假象）。**[2026-08-15 驗收修正，第三輪：文件對齊與補測試]** 修正 Decision 4 與 `proposal.md` 沒跟上 `IsSoldBy` 新增與 Decision 9（Expired 不落地）的用詞落差；`tasks.md` 1.4/3.7/4.5 補上對應更新記錄；補上 `IsSoldBy` 的 Domain 單元測試，以及 `ConfirmOrderHandler` 對「解析出的座位屬於錯誤活動」的獨立測試（模擬呼叫端組錯 `seatsById` 字典的情境）。目前全 repo 測試：Domain.Tests 53、Application.Tests 45、WebApi.Tests 23，全數通過。唯一仍保留的已知缺口：`ticket-ordering` 的「票種事後調整票價不影響既有訂單」情境未被獨立測試覆蓋——因 `TicketType` 本階段未提供改價方法（Price 唯讀），該不變條件目前是由「快照為 decimal、`OrderItem` 不持有 `TicketType` 參照」的結構保證，而非透過實際改價後斷言驗證；待之後若替 `TicketType` 加上改價能力，需補上對應測試
