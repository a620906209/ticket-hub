...
4. Script 回傳：`expiredIds`（含原本的到期時間，供失敗補償用）、`promotedIds`（含新的入場逾時時間）
...
這個設計不需要引入 `decisionId`／版本號機制——`ZPOPMIN` 的原子性已保證同一 entryId 不會被重複決策，而 `Status` 欄位本身的單向流動特性，配合條件式 `UPDATE`，已完整提供「compare-and-set」需要的版本判定能力，不需要額外的版本欄位。
