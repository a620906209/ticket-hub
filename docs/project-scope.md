...
- ~~Redis 分散式鎖~~——**Leader Election 部分已完成實作並歸檔**（`purchase-queue-leader-election`，`openspec/changes/archive/2026-09-03-purchase-queue-leader-election/`）：`PurchaseQueueAdmissionService` 背景推進服務新增以 Redis `SET NX PX` + Lua compare-and-delete 實作的分散式鎖，多實例部署下每輪只讓一個實例真正執行推進；Redis 故障採 fail-open（照常執行並記錄 Warning）；`docker-compose.yml` 新增 `redis` 服務。新增 24＋2 項測試（單元／元件／服務層端到端，含 TTL 逾時重疊執行下的正確性、Redis 斷線後自動恢復互斥、應用程式啟動時 Redis 不可用仍可正常啟動等情境），4 個測試專案全數通過。實作完成、strict-reviewer PASS 後，人工複查另抓到 3 個 blocking 問題（AC↔測試追溯矩陣缺漏、PQLE-010 測試嚴謹度不足、新舊 spec 對「併發推進」保證範圍的字面矛盾）並已修正——spec-reviewer／strict-reviewer 的 PASS 判定不是天花板，仍需人工複查。`purchase-queue`／新增 `purchase-queue-leader-election` 兩份 spec 已於歸檔時同步更新。已合併至 master、已歸檔並推送至遠端。**入場推進的併發控制改用 Redis 已完成實作並歸檔**（`purchase-queue-redis-admission`，`openspec/changes/archive/2026-09-17-purchase-queue-redis-admission/`）：`PurchaseQueueAdmissionService` 的入場推進互斥機制從 Postgres `GetForAdmissionAsync` 悲觀鎖改為 Redis Lua Script 原子操作（`waiting`/`admitted` Sorted Set + `ZPOPMIN`），Postgres 仍是持久化真相來源，所有寫回改為條件式 `UPDATE`；新增 Redis 鏡像逐筆校正機制處理 Redis 重啟/同步失敗，Redis 不可用時改為 fail-closed（偏離既有 `purchase-queue-leader-election` 的 fail-open 慣例，因為此處 Redis 已是正確性機制本身而非純效能鎖）。提案階段 11 輪 spec-reviewer 審查、實作後 3 輪 strict-reviewer PASS，人工複查仍額外抓到 1 個 blocking 問題（入場推進 Lua Script 的 `ZPOPMIN` 對「校正 gap-fill 決策與寫入之間的時間差」造成的跨鏡像暫態重複沒有防禦，會非預期重設已入場者的逾時時間）與 2 個建議性問題（校正耗時縮短實際入場視窗、格式損壞殘留永久卡名額）並已修正——再次印證 PASS 判定不是天花板。`purchase-queue`／`purchase-queue-leader-election` 兩份既有 spec 已於歸檔時同步更新。已合併至 master，尚未推送至遠端。Queue 排隊室（座位鎖定機制進階版）的 Redis 資料結構重寫維持未排定，不在本次範圍
- 多租戶主辦方管理介面（審核、切換）
- 實名制驗證（姓名、身分證末四碼或手機號）
- ~~動態驗證碼（CAPTCHA）~~——**已完成實作並歸檔，已合併至 master 並推送至遠端**（註冊、登入、加入排隊三個端點補上圖形驗證碼：`SixLabors.ImageSharp` 純受管理程式碼繪製 4 碼英數字圖片（答案字元使用 `RandomNumberGenerator` 密碼學安全亂數產生），答案雜湊暫存於 Redis、一次性核對（`GETDEL` 原子操作），Redis 故障時 fail-closed（與既有 Redis 用途的 fail-open 慣例相反）；`GET /api/captcha` 另掛獨立命名的 `captcha` rate-limit policy，與既有 `api-rate-limiting` 能力的 `login` policy 共用 `Microsoft.AspNetCore.RateLimiting` 既定機制與回應格式，但各自獨立計數、不共用額度；新增獨立的 `ErrorType.CaptchaInvalid` 讓前端能正確區分「驗證碼答案錯誤」與其他語意的 400。實作完成、strict-reviewer PASS 後，人工複查另抓到多輪問題（CAPTCHA-CANCEL-004 測試手法從 Redis SCAN 前後比對改為 mock 直接驗證、驗證碼答案原誤用 `Random.Shared` 改為密碼學安全亂數、前端原依泛用 400 狀態碼判斷驗證碼錯誤改為依可區分的 ErrorType）並已修正——同上，spec-reviewer／strict-reviewer 的 PASS 判定不是天花板，仍需人工複查。`authentication`／`member-management`／`purchase-queue`／`buyer-web-ui` 四份既有 spec 與新增的 `captcha-verification` spec 已於歸檔時同步更新，見 `openspec/changes/archive/2026-09-15-captcha-verification/`）
...
- ~~快取層（Redis 等，一般查詢 API 用）~~——**已完成實作，已合併至 master 並推送至遠端**（`GET /api/events`、`GET /api/events/{id}/ticket-types` 採 Redis cache-aside 快取，建立活動/票種、切換熱門搶購模式時明確失效，另有 TTL 安全網與 fail-open 邊界；見 `openspec/changes/archive/2026-09-12-query-caching/`）
...
- 銷售報表為活動建立後即可查詢的即時查詢，不限制活動開始時間，歷史趨勢分析列入 Won't
...
**Phase 1 Must 盤點快照（2026-08-26，`count-ticket-type-web-ui` 完成後更新——Phase 1 Must 全數完成）**
