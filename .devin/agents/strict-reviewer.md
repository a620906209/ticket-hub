...
- [ ] 變更的程式碼註解或 design.md 若斷言「這種情況由既有的某個全域/共用
      ...
      確實存在,就採信斷言為真
- [ ] 變更的程式碼註解、commit message 或測試斷言若宣稱「這個競態是刻意
      接受的,因為後續會自我收斂/冪等」,MUST 獨立追查**收斂發生前**、狀態
      不一致期間,是否有其他呼叫路徑會讀取並使用這個不一致的中間狀態
      (例如另一段邏輯對同一份資料做盲寫、無 compare-and-swap 的更新)。
      若存在這樣的路徑,即使程式碼作者已經論證「最終會收斂」,也要視為
      blocking,不得因為「作者已經分析過是安全的」就採信。特別注意「依
      快照做出的決策」與「決策對應的寫入動作」若不是同一個原子操作,中間
      的時間差本身就是一個獨立的競態視窗,需要單獨檢查
...
的不變量(還原修正後測試仍會通過)、「自我收斂/冪等」的競態安全性斷言未經
獨立查證即採信(即存在會讀取並使用收斂前不一致狀態的路徑),一律 blocking;
純 DTO/mapping/wiring 缺獨立測試,若已有更高層測試覆蓋,為 warning 或不列出。
