...
function mountPage() {
  return mount(EventListPage, {
    global: {
      plugins: [ElementPlus],
      stubs: { ElSelect: ElSelectStub, ElOption: ElOptionStub, ElDatePicker: ElDatePickerStub },
    },
  })
}

describe('EventListPage 建立活動：場館／座位圖下拉選單', () => {
  beforeEach(() => {
  ...
  it('建立活動成功後，場館／座位圖下拉選單的選擇值一併清空', async () => {
    vi.mocked(adminApi.getVenues).mockResolvedValue([{ id: 'venue-a', name: 'Venue A' }])
    vi.mocked(adminApi.getVenueById).mockResolvedValue({
      id: 'venue-a',
      name: 'Venue A',
    ...
    expect((venueSelect.element as HTMLSelectElement).value).toBe('')
    expect((seatMapSelect.element as HTMLSelectElement).value).toBe('')
    expect(seatMapSelect.findAll('option')).toHaveLength(1)
  })
})
